...
public static class OrderEndpoints
{
    public static void MapOrderEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        // ====================================================================
        group.MapPost("/", async (CreateOrderRequest request, ClaimsPrincipal user, AppDbContext db) =>
        {
        ...
        // ====================================================================
        group.MapGet("/{orderId:guid}", async (Guid orderId, ClaimsPrincipal user, AppDbContext db) =>
        {
            var userIdString = user.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            var customerId = Guid.Parse(userIdString!);

            // Dùng .Include() để EF Core "chui" sang bảng OrderDetail lấy danh sách sản phẩm
        ...
        // ====================================================================
        group.MapPut("/{orderId:guid}/cancel", async (Guid orderId, ClaimsPrincipal user, AppDbContext db) =>
        {
            var userIdString = user.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            var customerId = Guid.Parse(userIdString!);
