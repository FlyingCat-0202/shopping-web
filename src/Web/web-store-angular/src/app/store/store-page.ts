...
import { CatalogViewComponent } from '../catalog-view/catalog-view';
...
import { OrderDetailModalComponent } from '../order-detail-modal/order-detail-modal';
import { ProductDetailComponent } from '../product-detail/product-detail';
import { ProductEditorModalComponent } from '../product-editor-modal/product-editor-modal';
import { StoreHeaderComponent } from '../store-header/store-header';
import { NotificationSummary, NotificationsFacade } from './notifications.facade';
...
export class StorePage implements OnInit {
  ...
  readonly notifications = this.notificationsFacade.notifications;
  readonly unreadNotificationCount = this.notificationsFacade.unreadCount;
  readonly notificationsLoading = this.notificationsFacade.loading;
  readonly notificationUnreadOnly = this.notificationsFacade.unreadOnly;
  ...
  readonly relatedProducts = computed(() => {
    ...
    const seenProductIds = new Set<string>([selectedProduct.id]);
    const candidates = [
      ...this.products(),
      ...this.searchResults(),
      ...Object.values(this.productCache()),
    ];

    return candidates
      .filter((product) => {
        if (seenProductIds.has(product.id)) return false;
        if (product.categoryId !== selectedProduct.categoryId) return false;
  ...
  readonly cartQuantityForDetail = (productId: string): number => this.cartQuantity(productId);
  readonly addButtonTextForDetail = (product: Product): string => this.addButtonText(product);
  readonly isDeletingProductForCatalog = (product: Product): boolean => this.isDeletingProduct(product);
  readonly isNotificationBusyForDrawer = (notification: NotificationSummary): boolean =>
    this.isNotificationBusy(notification);
  readonly formatPriceForDrawer = (value: number): string => this.formatPrice(value);
  readonly formatDateForDrawer = (value?: string): string => this.formatDate(value);
  readonly paymentForDrawer = (order: OrderSummary): PaymentSummary | undefined => this.paymentFor(order);
  readonly canConfirmPaymentForDrawer = (order: OrderSummary): boolean => this.canConfirmPayment(order);
  readonly isConfirmingPaymentForDrawer = (order: OrderSummary): boolean => this.isConfirmingPayment(order);
  readonly payButtonTextForDrawer = (order: OrderSummary): string => this.payButtonText(order);
  readonly isOrderActionBusyForDrawer = (order: OrderSummary, action: string): boolean =>
    this.isOrderActionBusy(order, action);
  readonly isOrderDetailBusyForDrawer = (order: OrderSummary): boolean => this.isOrderDetailBusy(order);
  readonly isMockingPaymentForDrawer = (payment: PaymentSummary, success: boolean): boolean =>
    this.isMockingPayment(payment, success);
  readonly canMockPaymentForDrawer = (payment: PaymentSummary): boolean => this.canMockPayment(payment);
  readonly paymentStatusClassForDrawer = (status: string): string => this.paymentStatusClass(status);
  readonly shortIdForModal = (value?: string): string => this.shortId(value);
  ...
  async ngOnInit(): Promise<void> {
    this.route.paramMap
      ...
      .subscribe((params) => {
        void this.syncProductDetailFromRoute(params.get('productId'));
      });
  ...
  setNotificationUnreadOnly(value: boolean): void {
    this.notificationsFacade.setUnreadOnly(value);
  ...
  onProductCardKeydown(event: KeyboardEvent, product: Product): void {
  ...
  productDetailUrl(product: Product): string {
  ...
  isNotificationBusy(notification: NotificationSummary): boolean {
    return this.notificationsFacade.isBusy(notification);
  }

  isMarkingAllNotificationsRead(): boolean {
    return this.notificationsFacade.isMarkingAllRead();
  }
...
private normalizeOrderDetail(order: any): OrderDetail {
  ...
  private async syncProductDetailFromRoute(productId: string | null, showErrors = true): Promise<void> {
    if (!productId) {
      this.selectedProduct.set(null);
      return;
    }

    const cachedProduct = this.findProduct(productId);
    if (cachedProduct) {
    ...
    try {
      ...
      if (!product) {
        this.selectedProduct.set(null);
        if (showErrors) this.showToast('Product not found.');
        return;
      }
